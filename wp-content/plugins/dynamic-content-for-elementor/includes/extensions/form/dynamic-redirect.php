...
class DynamicRedirect extends \ElementorPro\Modules\Forms\Classes\Action_Base
{
    ...
    public function run($record, $ajax_handler)
    {
        ...
        if (!empty($settings['dce_form_redirect_repeater'])) {
            $settings['dce_form_redirect_repeater'] = \array_map(function ($value) use($fields, $record) {
                return Plugin::instance()->text_templates->expand_shortcodes_or_callback($value, ['form-fields' => $record->get('fields')], function ($str) use($fields) {
                    return Helper::get_dynamic_value($str, $fields);
                });
            }, $settings['dce_form_redirect_repeater']);
        }
