...
class Export extends \ElementorPro\Modules\Forms\Classes\Action_Base
{
    ...
    public function register_settings_section($widget)
    {
        ...
        $repeater_fields->add_control('dce_form_export_field_value', ['label' => esc_html__('Field Value', 'dynamic-content-for-elementor'), 'description' => esc_html__('It\'s the value of the parameter in the request', 'dynamic-content-for-elementor') . '<br>?field_key=<b>FieldValue</b>&page=<b>2</b>&txt=<b>Test</b><br>' . esc_html__('Can use static text, field Shortcode, Token or mixed', 'dynamic-content-for-elementor'), 'type' => Controls_Manager::TEXT]);
    ...
    public function run($record, $ajax_handler)
    {
        ...
        foreach ($repeater_keys_to_expand as $key) {
            if (isset($settings[$key]) && \is_array($settings[$key])) {
                $settings[$key] = \array_map(function ($item) use($fields, $record) {
    ...
    protected function export($fields, $settings, $ajax_handler, $record)
    {
        ...
        if (!empty($settings['dce_form_export_fields'])) {
            foreach ($settings['dce_form_export_fields'] as $akey => $adata) {
                $pvalue = Plugin::instance()->text_templates->expand_shortcodes_or_callback($adata['dce_form_export_field_value'], ['form-fields' => $record->get('fields')], function ($str) use($fields) {
                    return Helper::get_dynamic_value($str, $fields);
                });
