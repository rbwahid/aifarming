...
class PdfGenerator extends \ElementorPro\Modules\Forms\Classes\Action_Base
{
    ...
    public function set_pdf_url_and_path($record)
    {
        ...
        $dir_rel_path = Plugin::instance()->text_templates->expand_shortcodes_or_callback($record->get_form_settings('dce_form_pdf_folder'), ['form' => $fields], function ($str) use($fields) {
        ...
        $file_name = Plugin::instance()->text_templates->expand_shortcodes_or_callback($record->get_form_settings('dce_form_pdf_name'), ['form' => $fields], function ($str) use($fields) {
            return Helper::get_dynamic_value($str, $fields);
        });
    ...
    public function register_settings_section($widget)
    {
        ...
        $widget->add_control('dce_form_pdf_converter', ['label' => esc_html__('Converter', 'dynamic-content-for-elementor'), 'type' => Controls_Manager::SELECT, 'description' => esc_html__('Choose the converter that will generate the PDF.', 'dynamic-content-for-elementor'), 'options' => ['svg' => 'SVG', 'html' => 'HTML'], 'toggle' => \false, 'default' => 'svg']);
        $warning = esc_html__('The SVG Converter is only for simple PDFs. It has problems with languages other than english and input text is not wrapped. We recommend the newer HTML converter, it doesn’t have these limitations.', 'dynamic-content-for-elementor');
        ...
        //      'raw' => '<div id="elementor-panel__editor__help" class="p-0"><a id="elementor-panel__editor__help__link" href="' . $this->get_docs() . '" target="_blank">' . esc_html__( 'Need Help', 'dynamic-content-for-elementor' ) . ' <i class="eicon-help-o"></i></a></div>',
        //      'separator' => 'before',
        //  ]
        // );
        $widget->end_controls_section();
            ...
            if ($settings['dce_form_pdf_converter'] === 'dompdf') {
                ...
                foreach ($fields_to_expand as $field) {
                    if (isset($settings[$field])) {
                        $settings[$field] = Plugin::instance()->text_templates->expand_shortcodes_or_callback($settings[$field], ['form' => $fields], function ($str) use($fields) {
                            return Helper::get_dynamic_value($str, $fields);
                        });
                    }
                }
    ...
    private function svg_converter($settings, $record, $fields, $ajax_handler)
    {
        ...
        $tfun = function ($svg) use($record, $fields) {
            ...
            return Plugin::instance()->text_templates->expand_shortcodes_or_callback($svg, ['form' => $fields], function ($str) use($fields) {
                return Helper::get_dynamic_value($str, $fields);
            });
        };
    ...
    public function dompdf_converter($settings, $fields, $ajax_handler = null)
    {
        ...
        $pdf_html = Plugin::instance()->text_templates->expand_shortcodes_or_callback($pdf_html, ['form' => $fields], function ($str) use($fields) {
            return Helper::get_dynamic_value($str, $fields);
        });
        // add CSS
