...
class DynamicEmail extends \ElementorPro\Modules\Forms\Classes\Action_Base
{
    ...
    public function register_settings_section($widget)
    {
        ...
        $repeater_fields->add_control('dce_form_email_attachments', ['label' => esc_html__('Add Upload files as Attachments', 'dynamic-content-for-elementor'), 'type' => Controls_Manager::SWITCHER, 'description' => esc_html__('Send all Uploaded Files as Email Attachments', 'dynamic-content-for-elementor')]);
        $repeater_fields->add_control('dce_form_email_attachments_delete', ['label' => esc_html__('Delete Files after Emails are sent', 'dynamic-content-for-elementor'), 'type' => Controls_Manager::SWITCHER, 'description' => esc_html__('Remove all uploaded Files from Server after Email is sent with the Files as Attachments', 'dynamic-content-for-elementor'), 'condition' => ['dce_form_email_attachments!' => '']]);
        $repeater_fields->add_control('dce_form_pdf_attachments_delete', ['label' => esc_html__('Delete PDF attachments after Emails are sent', 'dynamic-content-for-elementor'), 'type' => Controls_Manager::SWITCHER, 'default' => 'no', 'description' => esc_html__('Remove all attached PDF files from Server after Email is sent.', 'dynamic-content-for-elementor')]);
        ...
        foreach ($settings['dce_form_email_repeater'] as $mkey => $amail) {
            if ($amail['dce_form_email_enable']) {
                ...
                if ($use_template) {
                    ...
                    $dce_form_email_content = Plugin::instance()->text_templates->dce_shortcodes->call_with_data(['form-fields' => $record->get('fields')], [$template_system, 'build_elementor_template_special'], [$atts]);
                    ...
                    $dce_form_email_to = Plugin::instance()->text_templates->expand_shortcodes_or_callback($email_fields['dce_form_email_to'], ['form-fields' => $record->get('fields')], $callback);
                    $dce_form_email_subject = Plugin::instance()->text_templates->expand_shortcodes_or_callback($email_fields['dce_form_email_subject'], ['form-fields' => $record->get('fields')], $callback);
