...
class License
{
    public static function show_license_form()
    {
        ...
        ?>">
			<?php 
        $is_license_active ? submit_button(esc_html__('Deactivate', 'dynamic-content-for-elementor'), 'cancel') : submit_button(esc_html__('Save key and activate', 'dynamic-content-for-elementor'));
        ?>
			</form>
		<?php 
                ...
                ?>
					<p>
						<strong style="color:#46b450;">
							<?php 
                /* translators: %1$s: last 4 digits of the license key */
                \printf(esc_html__('Your license ending in \'%1$s\' is valid and active.', 'dynamic-content-for-elementor'), esc_html(Plugin::instance()->license_system->get_license_key_last_4_digits()));
                ?>
						</strong>
					</p>
				<?php 
        ...
        if ($is_license_active) {
            ...
            ?></label>
					...
					<?php 
            submit_button(esc_html__('Save my preference', 'dynamic-content-for-elementor'));
            ?>
				</form>
			</div>
            ...
            ?></h4>
					<p>
						<?php 
            /* translators: %s: version number of Dynamic Content for Elementor */
            \printf(esc_html__('Experiencing an issue with Dynamic.ooo - Dynamic Content for Elementor version %s? Rollback to a previous version before the issue appeared.', 'dynamic-content-for-elementor'), DCE_VERSION);
            ?>
					</p>
            ...
            if (!empty($rollback_versions)) {
                ...
                ?>
						...
						<?php 
                submit_button(esc_html__('Rollback now', 'dynamic-content-for-elementor'));
            } else {
