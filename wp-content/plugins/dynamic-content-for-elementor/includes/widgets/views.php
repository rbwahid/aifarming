        ...
        if (!empty($objects)) {
            ...
            foreach ($objects as $key => $dce_obj) {
                ...
                if (!empty($settings['dce_views_group_by_field'])) {
                    ...
                    $group_value = Plugin::instance()->text_templates->expand_shortcodes_or_callback($settings['dce_views_group_by_field_heading'], ['heading' => $group_value, 'permalink' => $term_permalink], function ($str) use($group_value, $term_permalink) {
                ...
                foreach ($settings['dce_views_order_by'] as $key => $asort) {
                    if ($asort['dce_views_order_field']) {
                        ...
                        if (!\array_key_exists($field_name, $default_fields) && !\array_key_exists(\str_replace('post_', '', $field_name), $default_fields)) {
                            ...
                            if (isset($args['orderby']) && \is_array($args['orderby']) && \is_array($asort)) {
