...
class AcfFields extends \DynamicContentForElementor\Widgets\WidgetPrototype
{
    ...
    protected function safe_register_controls()
    {
        ...
        $this->add_control('acf_dynamic', ['label' => esc_html__('Apply Shortcodes', 'dynamic-content-for-elementor'), 'type' => Controls_Manager::SWITCHER, 'condition' => ['acf_type' => ['text', 'textarea', 'wysiwyg']]]);
        ...
        $this->add_control('acf_field_url', ['label' => esc_html__('ACF Field URL', 'dynamic-content-for-elementor'), 'type' => Controls_Manager::SELECT, 'groups' => $acf_url_groups, 'default' => esc_html__('Select the field...', 'dynamic-content-for-elementor'), 'condition' => ['link_to' => 'acf_url']]);
    ...
    protected function safe_render()
    {
        ...
        if ('list' === $typeField) {
            ...
            $field_value = \get_field($idFields, $id_page ? $id_page : null);
            ...
            if ($output == '') {
                ...
                $d = \DateTime::createFromFormat($dataDate['return_format'], $output);
                if ($d) {
                    $locale = get_locale();
                    if (!\strpos($locale, '.')) {
                    ...
                    $output = \strftime($date_format, \intval($d->format('U')));
                } else {
            ...
            $settings['html_tag'] = 'div';
            $imageAlt = esc_attr($imageField['alt']);
            $imageDesc = wp_kses_post($imageField['description']);
            $imageCapt = wp_kses_post($imageField['caption']);
