...
class AcfFrontend extends \DynamicContentForElementor\Widgets\WidgetPrototype
{
    ...
    protected function safe_render()
    {
        ...
        switch ($settings['retrieve_from']) {
            case 'current_post':
                ...
                break;
            case 'other_post':
                $post_id = $settings['retrieve_from_id'];
                if (empty($post_id) || !get_post($post_id)) {
                    Helper::notice(\false, esc_html__('Please select the post', 'dynamic-content-for-elementor'), 'elementor-alert-danger');
                    return;
                }
        ...
        if (!$this->has_fields_to_show($args)) {
            Helper::notice(\false, esc_html__('You have no fields to show', 'dynamic-content-for-elementor'), 'elementor-alert-danger');
            return;
        }
        acf_form($args);
    }
    ...
    protected function get_return_value($settings)
    {
        if ('redirect' === $settings['after_submit']) {
            switch ($settings['url_target']) {
                ...
                case 'another':
                    return esc_url($settings['redirect']['url'] ?? \false);
            }
        }
        return \false;
    }
    ...
    protected function has_fields_to_show($args)
    {
        ...
        if (isset($args['field_groups']) && !empty($args['field_groups']) && $args['field_groups'][0] !== null) {
            return \true;
        }
        return \false;
    }
