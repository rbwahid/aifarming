...
class AcfSlider extends \DynamicContentForElementor\Widgets\WidgetPrototype
{
    ...
    protected function safe_register_controls()
    {
        ...
        $this->add_control('lightbox_type', ['label' => esc_html__('Lightbox Type', 'dynamic-content-for-elementor'), 'type' => Controls_Manager::SELECT, 'options' => ['' => esc_html__('Default', 'dynamic-content-for-elementor'), 'photoswipe' => 'Photoswipe'], 'default' => '', 'condition' => ['enable_lightbox' => 'yes']]);
    ...
    protected function safe_render()
    {
        ...
        $direction = ' dce-direction-horizontal';
        $swiper_class = \Elementor\Plugin::$instance->experiments->is_feature_active('e_swiper_latest') ? 'swiper' : 'swiper-container';
        ...
        foreach ($acf_gallery as $image) {
            if (!isset($image['id'])) {
                $img_id = $image;
                $image = Helper::get_attachment($img_id);
            }
            $img_id = $image['id'];
            $img_url = $image['url'];
            $img_alt = $image['alt'];
            ...
            if ($settings['use_bg_image'] != '') {
                $bg_image_style = ' style="background-image: url(' . esc_url($image_url) . '); background-repeat: no-repeat; background-size: cover;"';
            ...
            if ($settings['use_bg_image'] == '') {
                echo '<img src="' . esc_url($image_url) . '" itemprop="thumbnail" alt="' . esc_attr($img_alt) . '" />';
