...
trait Image
{
    ...
    public static function get_attachment($attachment_id)
    {
        ...
        return ['id' => $attachment_id, 'alt' => esc_attr(get_post_meta($attachment_id, '_wp_attachment_image_alt', \true)), 'caption' => wp_kses_post($attachment->post_excerpt), 'description' => wp_kses_post($attachment->post_content), 'href' => get_attachment_link($attachment_id), 'src' => $img_src[0], 'title' => esc_html($attachment->post_title), 'url' => $img_src[0], 'width' => $img_src[1], 'height' => $img_src[2]];
    }
}
