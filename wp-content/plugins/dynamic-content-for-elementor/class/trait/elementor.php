...
trait Elementor
{
    ...
    public static function set_all_settings_by_id($element_id = null, $settings = array(), $post_id = null)
    {
        ...
        if ($element_id) {
            ...
            $post_meta = \DynamicContentForElementor\Helper::set_array_value_by_keys(\is_array($post_meta) ? $post_meta : [], $keys_array, $settings);
    ...
    public static function set_settings_by_id($element_id, $key, $value = null, $post_id = null)
    {
        ...
        if (!empty($keys_array)) {
            ...
            $keys_array[] = $key;
            $post_meta = \DynamicContentForElementor\Helper::set_array_value_by_keys(\is_array($post_meta) ? $post_meta : [], $keys_array, $value);
            \array_walk_recursive($post_meta, function ($v, $k) {
                $v = self::escape_json_string($v);
            });
            $post_meta_prepared = wp_json_encode($post_meta);
