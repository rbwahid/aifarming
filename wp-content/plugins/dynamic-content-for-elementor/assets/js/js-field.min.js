...
return""}
let makeIsFieldActiveFunction=(form)=>{return(id)=>{let el=form.getElementsByClassName(`elementor-field-group-${id}`);if(!el.length){throw `Field ${id} was not found`}
...
onChange();form.addEventListener(realTime?'input':'change',onChange);jQuery(form).on('conditions-done',onChange)}
