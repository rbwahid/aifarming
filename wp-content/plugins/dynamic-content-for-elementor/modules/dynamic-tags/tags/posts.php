...
class Posts extends Tag
{
        ...
        if ($wp_query->have_posts()) {
            while ($wp_query->have_posts()) {
                ...
                if ('yes' === $settings['link']) {
                    $this->set_render_attribute('link', 'href', get_the_permalink() ?: '#');
    ...
    {
        $settings = $this->get_settings_for_display();
        $args = ['post_type' => \DynamicContentForElementor\Helper::validate_post_types($settings['post_type']), 'posts_per_page' => $settings['posts'], 'order' => $settings['order'], 'orderby' => $settings['orderby'], 'post_status' => $settings['post_status']];
